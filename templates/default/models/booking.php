...
if(isset($_POST['destination_id']) && is_numeric($_POST['destination_id'])){
    ...
    $_SESSION['meal_name'] = $_POST['meal_name'];
...
if(count($field_notice) == 0){
    ...
    if($num_nights <= 0) $msg_error .= $texts['NO_AVAILABILITY'];
        ...
        $res_hotel = getRoomsResult($from_time, $to_time, $_SESSION['num_adults'], $_SESSION['num_children'],false,0,$_SESSION['meal_name']);
                                            ...
                                            <div class="boxed">
                                                ...
                                                if($result_room !== false ){
                                                    ...
                                                    foreach($result_room as $row){
                                                        ...
                                                        $meal_name_session=$_SESSION['meal_name'];

                                                        //for test
                                                            $result_room_rate_sr = $db->prepare('SELECT pm_rate_meal_price.meal_name FROM ((pm_rate_meal_price INNER JOIN pm_rate ON pm_rate_meal_price.id_rate = pm_rate.id) INNER JOIN pm_room ON pm_room.id = pm_rate.id_room) where pm_room.id = ? AND pm_rate_meal_price.meal_name=? GROUP by pm_rate_meal_price.meal_name');
                                                            $result_room_rate_sr->bindParam(1, $id_room, PDO::PARAM_INT);
                                                            $result_room_rate_sr->bindValue(2, $meal_name_session, PDO::PARAM_STR);
                                                           
                                                            if($result_room_rate_sr->execute() !== false && $db->last_row_count() > 0){
                                                                
                                                                 $row_test = $result_room_rate_sr->fetch();
                                                                 if($meal_name_session==$row_test['meal_name']){
                                                                    $meal_plane_from_rate=$meal_name_session;
                                                                 }
                                                                 else{
                                                                    $meal_plane_from_rate=$row['meal_plan'];
                                                                 } 
                                                              
                                                            } 
                                                            else{
                                                                $meal_plane_from_rate='null_meal';
                                                            }  
                                                        ...
                                                        <div class="row room-result">
                                                            ...
                                                            <div class="<?php echo (SHOW_CALENDAR == 1) ? 'col-lg-2 col-md-3 col-sm-3' : 'col-lg-3 col-md-5 col-sm-5'; ?> text-center sep">
                                                                ...
                                                                <?php echo $texts['MEAL_PLAN']; ?> : <i class="fas fa-fw fa-cheese"></i><?php echo $texts["$meal_plane_from_rate"]  ?>
                                                                <?php
                                                                if($room_stock > 0){ ?>
