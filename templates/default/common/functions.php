...
function getRoomsResult($from_time, $to_time, $adults, $arr_children, $strict = false, $room_id = 0,$meal_name='null')
{
    global $db;
    global $texts;
    global $room_ids;
    ...
    // sameh edit
    //echo 'console.log("'.$meal_name.'")';
            $result_meal_rate = $db->prepare('SELECT * FROM pm_rate_meal_price WHERE id_rate = :rate_id and meal_name = :meal_name');
            $result_meal_rate->bindParam(':rate_id', $rate_id);
            $result_meal_rate->bindParam(':meal_name', $meal_name);
   ...
   // end sameh edit
        ...
        foreach($result_rate as $i => $row){
            ...
            $num_people = $adults+$children;
                ...
                // sameh edit
                    ...
                    if($result_meal_rate->execute() !== false && $db->last_row_count() > 0){
                         $row_meal = $result_meal_rate->fetch();
                         $price = $row_meal['meal_price'];
                    }
                    else{
    ...
    //                     $price += $extra_price;
                        ...
                        if(is_array($arr_children)){
                             $count_child_array=0;
                            $count_child_array=count($arr_children);
                            // echo 'console.log("'.$count_child_array.'")';
    
                            for($j = 0; $j < $count_child_array; $j++){
                                     $age = is_array($arr_children) ? $arr_children[$j] : '0';
                                    if($result_child_rate->execute() !== false && $db->last_row_count() > 0){
                                        $row = $result_child_rate->fetch();
                                        $extra_price += $row['price'];
